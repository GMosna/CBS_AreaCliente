...
export function DiscountModal({ parceiro, open, onClose }: DiscountModalProps) {
  ...
  async function copiarDesconto() {
    await navigator.clipboard.writeText(parceiro.desconto_descricao);
    setCopied(true);
    setTimeout(() => setCopied(false), 2500);
  }

  function abrirWhatsApp() {
    if (!parceiro.whatsapp) return;
    const msg = encodeURIComponent(
      `Olá! Sou inquilino da Sassi Imóveis e gostaria de utilizar o benefício da ${parceiro.nome_empresa}.`
    );
    window.open(`https://wa.me/55${parceiro.whatsapp}?text=${msg}`, '_blank');
  }
      ...
      >
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="bg-[#981c1c]/15 border border-[#e43333]/30 rounded-2xl p-5 mb-6">
            ...
            {parceiro.frequencia_desconto && (
              <p className="text-[#9ca3af] text-sm mt-3 flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5 text-[#e43333]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
